...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.visitedCorners = []
        self.extendState = (self.startingPosition, self.visitedCorners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # 2.element is visitedCorners
            x, y = currState
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                # TODO find out if we should make the cost function generic
                # or just like that hard coded
                nextState = (nextx, nexty)
                if nextState in self.corners and nextState not in visitedCorners:
                    expand = visitedCorners + [nextState]
                    successors.append(((nextState, expand), action, 1))
                else:
...
def cornersHeuristic(state, problem):
    ...
    while notVisitedCorners:
        (dist, corner) = min([(util.manhattanDistance(currState, corner), corner) for corner in notVisitedCorners])
        currState = corner
