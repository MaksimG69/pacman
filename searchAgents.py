...
class CornersProblem(search.SearchProblem):
	...
	def getStartState(self):
		...
		"""
		"*** YOUR CODE HERE ***"
		# return a starting position and an list with corners
	...
	def isGoalState(self, state):
		...
		if len(state[1]) < 4:
			...
			for s in state[1]:
				if s not in self.corners: return False
		return True
		util.raiseNotDefined()
	...
	def getSuccessors(self, state):
		...
		for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
			...
			if not hits_wall:
				next_state_coordinates = (nextx, nexty)
				# try cost without function
				cost = 1
				if next_state_coordinates in self.corners and next_state_coordinates not in visited_corners:
					visited_corners.append(next_state_coordinates)
				successor = ((next_state_coordinates, visited_corners), action, cost)
...
def cornersHeuristic(state, problem):
	...
	visited_corners = state[1][:]
	# create unvisited corner
	unvisited = []
	for cr in corners:
		if cr not in visited_corners:
			unvisited.append(cr)
	heuristic_sum = 0
	# compute heuristic
	trigger = 0
	while len(unvisited) > 0:
		distance_and_corners = []
		for s in unvisited:
			if trigger <= 2:
		...
		heuristic_sum += distance_and_actual_state[0]
		unvisited.remove(actual_state)
